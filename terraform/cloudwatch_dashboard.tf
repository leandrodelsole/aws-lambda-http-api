#copied and adapted from https://github.com/FRosner/aws-lambda-monitoring-alerting-example/blob/master/terraform/cloudwatch_dashboard.tf
locals {
  ...
  project-name = "Java-Lambda-HTTP-API"
}
  ...
  "widgets": [
    {
      ...
      "height": 1,
      "properties": {
        "markdown": "# ${aws_lambda_function.insert-user.function_name}"
      }
    },
    {
      "type": "metric",
      ...
      "properties": {
        "metrics": [
          [
            "AWS/Lambda", "Duration",
            "FunctionName", "${aws_lambda_function.insert-user.function_name}",
            "Resource", "${aws_lambda_function.insert-user.function_name}",
            {
              "stat": "Maximum",
              "yAxis": "left",
        ...
        "yAxis": {
          "left": {
            "min": 0,
            "max": ${aws_lambda_function.insert-user.timeout}000,
            "label": "ms",
            "showUnits": false
          }
        },
    ...
    },
    {
      ...
      "properties": {
        ...
        "yAxis": {
          "left": {
            "min": 0,
            "max": ${aws_lambda_function.insert-user.memory_size},
            "showUnits": false,
            "label": "MB"
          }
        },
