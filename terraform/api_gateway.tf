...
resource "aws_api_gateway_method" "users-echo" {
  rest_api_id   = "${aws_api_gateway_rest_api.http-api.id}"
...
resource "aws_api_gateway_integration" "users-echo" {
  rest_api_id             = "${aws_api_gateway_rest_api.http-api.id}"
  resource_id             = "${aws_api_gateway_resource.echo.id}"
  http_method             = "${aws_api_gateway_method.users-echo.http_method}"
  integration_http_method = "POST" #must always be post when invoking lambda
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.region}:${var.account_id}:function:${aws_lambda_function.echo-user.function_name}/invocations"
  credentials             = "arn:aws:iam::${var.account_id}:role/${aws_iam_role.insert-user-exec.name}"
}
...
resource "aws_api_gateway_integration_response" "users-echo" {
  depends_on  = ["aws_api_gateway_integration.users-echo"]
  rest_api_id = "${aws_api_gateway_rest_api.http-api.id}"
  resource_id = "${aws_api_gateway_resource.echo.id}"
  http_method = "${aws_api_gateway_method.users-echo.http_method}"
...
resource "aws_api_gateway_deployment" "http-api" {
  depends_on = [
    "aws_api_gateway_integration.users-insert", "aws_api_gateway_integration.users-list", "aws_api_gateway_integration.users-echo"]
...
output "curl_echo" {
  value = "curl -H 'Content-Type: application/json' -d '{\"name\": \"Echo\", \"email\": \"ec@ho.com\"}' -X POST ${aws_api_gateway_deployment.http-api.invoke_url}${aws_api_gateway_resource.echo.path}/ ; echo"
}