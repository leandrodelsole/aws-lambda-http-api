#copied and adapted from https://github.com/FRosner/aws-lambda-monitoring-alerting-example/blob/master/terraform/cloudwatch_metric_filters.tf
...
resource "aws_cloudwatch_log_metric_filter" "calculator-memory" {
  ...
  log_group_name = "/aws/lambda/${aws_lambda_function.insert-user.function_name}"
