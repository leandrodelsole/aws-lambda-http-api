...
#echo user
resource "aws_lambda_function" "echo-user" {
  function_name = "EchoUserHttpApi"
...
resource "aws_iam_role_policy" "echo-user" {
  name = "lambda_policy"
  role = "${aws_iam_role.echo-user-exec.id}"
...
resource "aws_lambda_permission" "echo-user" {
  statement_id  = "AllowAPIGatewayInvokeEchoUser"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.echo-user.arn}"
  principal     = "apigateway.amazonaws.com"
